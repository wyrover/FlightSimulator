...
const glm::vec3& Quaternion::GetVector() const
{
	return m_vector;
}
...
Quaternion Quaternion::operator * (const Quaternion& rhs) const
{
	...
	scalar = m_scalar * rhs.GetScalar() - glm::dot(m_vector, rhs.GetVector());
	//scalar = m_scalar*rhs.GetScalar() - m_vector.DotProduct(rhs.GetVector());

	vec = glm::cross(m_vector, rhs.GetVector());
...
{
	m_vector = glm::vec3(vx, vy, vz);
	m_scalar = s;
}

void Quaternion::SetQuaternion(const glm::vec3& vec, float s)
{
	m_vector = vec;
	m_scalar = s;
}
